...
export class Controlador {
    constructor(initialData, onChange) {
        ...
        try {
            this.vistaUser = new CL_vAportes("");
        }
        catch (e) {
            console.error("Error al instanciar vista user:", e);
        }
        try {
            this.vistaAdmin = new CL_vDecanato("admin-");
        }
        catch (e) {
            console.error("Error al instanciar vista admin:", e);
        }
    ...
    setupEventListeners() {
        if (this.vistaUser) {
            ...
            this.vistaUser.btnAceptar.addEventListener("click", (e) => {
                ...
                if (!ok) {
                    alert("El ID ya existe. Elige otro ID.");
                    return;
                }
                if (this.onChange)
            ...
            this.vistaUser.btnEnviarReporte.addEventListener("click", () => {
                ...
                if (!text || this.currentUserReportTarget == null)
                    return;
    ...
    handleMostrarEstadisticas(vista) {
    ...
    actualizarModalEstadisticas(stats, vista) {
    ...
    handleAdminAceptar() {
        ...
        if (this.vistaAdmin.modoEdicion) {
            const originalId = (_a = this.currentAdminEditingId) !== null && _a !== void 0 ? _a : nuevoAp.id;
            const ok = this.decanato.actualizarAporteConId(originalId, nuevoAp);
            if (!ok) {
                alert("Error: ID duplicado o aporte original no encontrado.");
                return;
            }
    ...
    updateUI() {
        ...
        if (this.vistaUser)
            this.vistaUser.actualizarLista(filtrados, total);
        if (this.vistaAdmin)
            this.vistaAdmin.actualizarLista(filtrados, total);
    }
}
